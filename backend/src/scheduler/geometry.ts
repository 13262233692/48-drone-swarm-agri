...
export function computeClippedVoronoi(
  seeds: { droneId: string; x: number; y: number }[],
  clipPolygon: Point[],
): VoronoiCell[] {
  ...
  const delaunay = Delaunay.from(seeds, (s: any) => s.x, (s: any) => s.y)
...
export function generateBoustrophedonWaypoints(
  polygon: Point[],
  spacing: number,
): Point[] {
